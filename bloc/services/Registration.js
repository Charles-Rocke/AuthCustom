...
async function startWebAuthnRegistration(constructedPayload) {
	...
	try {
		...
		const resp = await fetch(`${import.meta.env.VITE_API_BLOC_API_URL}/api/signup?${queryParams}`)
		...
		const verificationResp = await fetch(`${import.meta.env.VITE_API_BLOC_API_URL}/api/verify_signup`, {
			method: 'POST',
			mode: 'cors',
			headers: {
